const validateFields = (response, requiredFields, featureName) => {
  ...
  if (missing.length > 0) {
    throw new Error(`AI response incomplete for ${featureName}.`)
  }
}

export const parseSituationResponse = (response) => {
  validateFields(response, ["legalCategory", "subcategory", "severity", "timeIsSensitive", "reasoning", "suggestedNextStep"], "Situation Finder")

  return {
    legalCategory: String(response.legalCategory),
    subcategory: String(response.subcategory),
    severity: ["High", "Medium", "Low"].includes(response.severity) ? response.severity : "Medium",
...
export const parseDocumentResponse = (response) => {
  validateFields(response, ["documentType", "flaggedClauses", "overallRisk", "summary"], "Document X-Ray")
  ...
  return {
    ...
    flaggedClauses: Array.isArray(response.flaggedClauses)
      ? response.flaggedClauses.map((clause) => ({
          ...
          severity: ["High", "Medium", "Low"].includes(clause.severity) ? clause.severity : "Medium",
          law: clause.law || null
        }))
      : [],
    missingProtections: Array.isArray(response.missingProtections) ? response.missingProtections.map(String) : [],
    userLeveragePoints: Array.isArray(response.userLeveragePoints) ? response.userLeveragePoints.map(String) : [],
    overallRisk: ["High", "Medium", "Low"].includes(response.overallRisk) ? response.overallRisk : "Medium",
    summary: String(response.summary)
  }
}

export const parseRightsResponse = (response) => {
  validateFields(response, ["identified", "violated", "evidenceToCollect", "immediateAction"], "Rights Navigator")
  ...
  return {
    ...
    violated: Array.isArray(response.violated)
      ...
      : [],
    evidenceToCollect: Array.isArray(response.evidenceToCollect) ? response.evidenceToCollect.map(String) : [],
  ...
  return {
    active: Array.isArray(response.active)
      ? response.active.map((d) => ({
          ...
          urgency: validUrgency.includes(d.urgency) ? d.urgency : "Medium",
          consequence: String(d.consequence || ""),
          actionRequired: String(d.actionRequired || ""),
          law: d.law || null
        }))
      : [],
    approaching: Array.isArray(response.approaching) ? response.approaching.map(String) : [],
    notes: String(response.notes || "")
  }
}

export const parseSignalResponse = (response) => {
  validateFields(response, ["letterType", "recipient", "subject", "body", "requestedAction"], "Signal Letter")
  ...
  return {
    ...
    legalCitations: Array.isArray(response.legalCitations) ? response.legalCitations.map(String) : [],
...
export const parseCourtPrepResponse = (response) => {
  validateFields(response, ["courtType", "whatToBring", "openingStatement", "keyArguments"], "Court Prep")

  return {
    courtType: String(response.courtType),
    whatToBring: Array.isArray(response.whatToBring) ? response.whatToBring.map(String) : [],
    openingStatement: String(response.openingStatement),
    keyArguments: Array.isArray(response.keyArguments) ? response.keyArguments.map(String) : [],
    likelyQuestions: Array.isArray(response.likelyQuestions)
      ...
      : [],
    opposingArguments: Array.isArray(response.opposingArguments) ? response.opposingArguments.map(String) : [],
    counterArguments: Array.isArray(response.counterArguments) ? response.counterArguments.map(String) : [],
    whatNotToSay: Array.isArray(response.whatNotToSay) ? response.whatNotToSay.map(String) : [],
    finalTip: String(response.finalTip || "")
  }
}

export const parseHealthCheckResponse = (response) => {
  validateFields(response, ["exposures", "recommendations", "overallHealthScore"], "Health Check")
  ...
  return {
    exposures: Array.isArray(response.exposures)
      ...
      : [],
    recommendations: Array.isArray(response.recommendations) ? response.recommendations.map(String) : [],
    areasToReview: Array.isArray(response.areasToReview) ? response.areasToReview.map(String) : [],
    overallHealthScore: ["Good", "Fair", "Needs Attention"].includes(response.overallHealthScore) ? response.overallHealthScore : "Fair",
    summary: String(response.summary || "")
  }
}
